...
def check_docu():
    ...
    publ_key_pharm = request.form['publ_key_p']
    priv_key_pharm = request.form['priv_key_p']
    fill_p = request.form['fill']

    document1 = blockchain.convert_document(document)

    public = blockchain.get_pair(publ_key_recipient)
    publ_key_recipient = str(public[1])

    print("public key recipient: ")
    print(publ_key_recipient)
    docu_hash = blockchain.check_document(publ_key_doctor, publ_key_recipient, document1)
    print("docu hash results:")
    print(docu_hash)
    if (str(document1[0:4]) == '0000') and (fill_p != 't'):
        print("zeros!")
        print(document1[4:])
        new_document1 = '9999' + document1[4:]
        print(new_document1)
        docu_hash1 = blockchain.check_document(publ_key_pharm, publ_key_recipient, new_document1)
        print("results of docu_hash1:")
        print(docu_hash1)
        if docu_hash1 == False:
            docu_hash2 = True
        else:
    ...
    if (fill_p == 't') and (str(document1[0:4]) == '0000'):
        new_document1 = '9999' + document1[4:]

        docu_hash3 = blockchain.sign_document(priv_key_pharm, publ_key_pharm, new_document1)

        index = blockchain.new_transaction(publ_key_pharm, publ_key_recipient, docu_hash3[0], docu_hash3[1])
    else:
        reponse = {
            'message': 'Not Valid Document to Fill',
        }



    if (docu_hash==True) and (docu_hash2==True):
        response = {
